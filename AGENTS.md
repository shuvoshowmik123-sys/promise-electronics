...
- Backend: Express.js 4 + TypeScript, Drizzle ORM v0.39, PostgreSQL (NeonDB via neon-http adapter)
- Auth: Firebase Admin SDK (server), Firebase client SDK (browser)
- AI: Groq (chat/audio/vision), Gemini 2.5 Flash (vision fallback)
- Deploy: Vercel (frontend + backend monorepo, serverless)
...
- DB DDL (CREATE TABLE, ALTER TABLE) must use raw `neon(url)` client, NOT Drizzle (known neon-http DDL bug)
...
- KG (knowledge graph): raw neon client in `server/brain/kg.service.ts`, lazy `getSql()` init
