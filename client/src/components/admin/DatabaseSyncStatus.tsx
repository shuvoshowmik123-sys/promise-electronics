...
export function DatabaseSyncStatus({ enabled = true, className }: DatabaseSyncStatusProps) {
  ...
  const ready = Boolean(data?.ready) && !isError;
  const label = ready ? "Live" : "Database connecting...";
  ...
  return (
    <div
      ...
      aria-live="polite"
    >
      {ready ? <CheckCircle2 className="h-3.5 w-3.5" /> : <CloudCog className="h-3.5 w-3.5" />}
      <span>{label}</span>
    </div>
  );
}
