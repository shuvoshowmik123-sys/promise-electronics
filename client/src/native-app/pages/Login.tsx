...
import { ArrowLeft, Smartphone, Lock, Loader2, Fingerprint } from "lucide-react";
...
export default function Login() {
    ...
    const { getAuth, saveAuth, checkAvailability } = useBiometrics();
    ...
    const handleBiometricLogin = async () => {
        ...
        if (creds && creds.username && creds.password) {
            setIsLoading(true);
            try {
                // The username stored is the full phone number
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Check if biometrics are available and not yet set up
            const isAvailable = await checkAvailability();
            if (isAvailable) {
                const wantsBiometrics = window.confirm("Would you like to enable biometric login for faster access next time?");
                if (wantsBiometrics) {
                    const saved = await saveAuth(fullPhone, password);
                    if (saved) {
                        toast({
                            title: "Biometrics Enabled",
                            description: "You can now log in with your fingerprint/face.",
                        });
                    }
                }
            }
        ...
        <NativeLayout className="bg-[var(--color-native-bg)] text-[var(--color-native-text)]">
            ...
            <div className="flex items-center justify-between px-6 py-4 pt-[calc(1rem+env(safe-area-inset-top))]">
                ...
                <div className="flex items-center gap-2 opacity-80">
                    <span className="text-xs font-bold tracking-widest uppercase text-[var(--color-native-text)]">Promise Electronics</span>
                </div>
                <div className="w-10" />
            </div>
                ...
                <form onSubmit={handleLogin} className="flex flex-col gap-5 w-full">
                    ...
                    <div className="flex gap-3">
                        <button
                            ...
                            className="relative flex-1 flex h-14 items-center justify-center gap-3 rounded-full border border-[var(--color-native-border)] bg-[var(--color-native-card)] text-[var(--color-native-text)] font-medium text-base hover:bg-[var(--color-native-input)] active:scale-[0.98] transition-all disabled:opacity-70"
                        >
                        ...
                        <button
                            ...
                            className="flex h-14 w-14 items-center justify-center rounded-full border border-[var(--color-native-border)] bg-[var(--color-native-card)] text-[var(--color-native-primary)] hover:bg-[var(--color-native-input)] active:scale-[0.95] transition-all"
                        >
