...
const RejectLeaveDialog = lazy(() => import("./salary/SalaryHRDialogs").then(m => ({ default: m.RejectLeaveDialog })));
const DismissHolidayDialog = lazy(() => import("./salary/SalaryHRDialogs").then(m => ({ default: m.DismissHolidayDialog })));
const AddHolidayDialog = lazy(() => import("./salary/SalaryHRDialogs").then(m => ({ default: m.AddHolidayDialog })));
const SalaryNatureSubTab = lazy(() => import("./salary/SalaryNatureSubTab").then(m => ({ default: m.SalaryNatureSubTab })));
const EmployeeCompensationSubTab = lazy(() => import("./salary/EmployeeCompensationSubTab").then(m => ({ default: m.EmployeeCompensationSubTab })));
const AdvisoryDashboardSubTab = lazy(() => import("./salary/AdvisoryDashboardSubTab").then(m => ({ default: m.AdvisoryDashboardSubTab })));
const OffboardingSubTab = lazy(() => import("./salary/OffboardingSubTab").then(m => ({ default: m.OffboardingSubTab })));
        ...
        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6 pb-24 md:pb-0">
            ...
            <motion.div variants={itemVariants}>
                <Tabs defaultValue="payroll" className="w-full">
                    <TabsList className="flex h-11 items-center justify-start rounded-full bg-slate-100 p-1 text-slate-500 w-full md:w-auto mb-6 flex-nowrap overflow-x-auto hide-scrollbar md:flex-wrap md:overflow-visible gap-1">
                        {[
                            ...
                            <TabsTrigger
                                key={value}
                                value={value}
                                className="shrink-0 whitespace-nowrap rounded-full px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium transition-all data-[state=active]:bg-white data-[state=active]:text-slate-900 data-[state=active]:shadow-sm gap-1.5"
                            >
                    ...
                    <TabsContent value="compensation">
                        <Suspense fallback={null}><EmployeeCompensationSubTab /></Suspense>
                    </TabsContent>

                    {/* ── Salary Nature ── */}
                    <TabsContent value="nature">
                        <Suspense fallback={null}><SalaryNatureSubTab /></Suspense>
                    </TabsContent>

                    {/* ── Advisory Dashboard ── */}
                    {isSuperAdmin && (
                        <TabsContent value="advisory">
                            <Suspense fallback={null}><AdvisoryDashboardSubTab /></Suspense>
                        </TabsContent>
                    )}

                    {/* ── Offboarding & Settlement ── */}
                    <TabsContent value="offboarding">
                        <Suspense fallback={null}><OffboardingSubTab /></Suspense>
                    </TabsContent>
