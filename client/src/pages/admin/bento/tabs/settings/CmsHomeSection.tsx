...
export type HomepageBrandId = number | string;
export interface HomepageBrand { id: HomepageBrandId; name: string; logoUrl: string; logoScale?: number; logoNormalizedAt?: string }
...
interface CmsHomeSectionProps {
    ...
    onUploadBrandLogo?: (id: HomepageBrandId, file: File) => Promise<void>;
    onAutoFitBrandLogo?: (id: HomepageBrandId) => Promise<void>;
    onAutoFitAllBrandLogos?: () => Promise<void>;
    autoFittingBrandIds?: HomepageBrandId[];
    isAutoFittingAllBrands?: boolean;
...
export default function CmsHomeSection({
    ...
    onUploadBrandLogo, onAutoFitBrandLogo, onAutoFitAllBrandLogos, autoFittingBrandIds = [], isAutoFittingAllBrands = false, onUploadProblemIcon,
    ...
    const updateBrand = (id: HomepageBrandId, field: 'name' | 'logoUrl' | 'logoScale', value: string | number) => {
        setHomepageBrands(homepageBrands.map(b => b.id === id ? { ...b, [field]: value } : b));
    };
    const addBrand = () => {
        if (newBrand.trim()) {
            setHomepageBrands([...homepageBrands, { id: Date.now(), name: newBrand.trim(), logoUrl: newBrandLogo.trim(), logoScale: 1 }]);
            setNewBrand("");
            setNewBrandLogo("");
        }
    };
        ...
        <div className="space-y-4 pb-20">
            ...
            >
                <div className="space-y-3">
                    ...
                    {homepageBrands.some(brand => brand.logoUrl?.trim()) && (
                        <div className="flex flex-col gap-2 rounded-xl border border-blue-100 bg-blue-50/60 p-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <p className="text-xs font-semibold text-blue-900">One-click logo fitting</p>
                                <p className="text-[11px] leading-relaxed text-blue-700">Auto Fit trims old logo files, centers them on a clean canvas, uploads the fixed PNG, and keeps the result after you save settings.</p>
                            </div>
                            <Button
                                type="button"
                                size="sm"
                                variant="outline"
                                onClick={() => onAutoFitAllBrandLogos?.()}
                                disabled={!onAutoFitAllBrandLogos || isAutoFittingAllBrands}
                                className="shrink-0 border-blue-200 bg-white text-blue-700 hover:bg-blue-50"
                            >
                                {isAutoFittingAllBrands ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <Sparkles className="mr-1.5 h-3.5 w-3.5" />}
                                Auto Fit All
                            </Button>
                        </div>
                    )}
                    ...
                    {homepageBrands.length === 0 ? (
                        ...
                        <div className="space-y-2">
                            {homepageBrands.map((brand) => (
                                <div key={brand.id} className="p-3 rounded-xl bg-slate-50 border border-slate-100 space-y-3 hover:border-blue-200 transition-colors">
                                        ...
                                        <div className="flex items-center gap-1 shrink-0">
                                            <label className="cursor-pointer inline-flex items-center gap-1 h-8 px-2.5 text-xs border border-slate-200 rounded-lg bg-white hover:bg-slate-50 text-slate-600 font-medium transition-colors">
                                                ...
                                                <input
                                                    ...
                                                    className="hidden"
                                                    onChange={(e) => {
                                                        if (e.target.files?.[0]) onUploadBrandLogo?.(brand.id, e.target.files[0]);
                                            ...
                                            {brand.logoUrl && (
                                                <Button
                                                    size="sm"
                                                    variant="outline"
                                                    onClick={() => onAutoFitBrandLogo?.(brand.id)}
                                                    disabled={!onAutoFitBrandLogo || autoFittingBrandIds.includes(brand.id)}
                                                    className="h-8 gap-1 px-2 text-xs text-blue-700 border-blue-100 bg-white hover:bg-blue-50"
                                                >
                                                    {autoFittingBrandIds.includes(brand.id) ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />}
                                    ...
                                    <div className="rounded-lg border border-slate-200 bg-white px-3 py-2">
                                        <div className="mb-1.5 flex items-center justify-between gap-2">
                                            <Label className="text-[11px] font-semibold text-slate-600">Logo scale</Label>
                                            <span className="font-mono text-[11px] text-slate-500">{Math.round((brand.logoScale ?? 1) * 100)}%</span>
                                        </div>
                                        <input
                                            type="range"
                                            min="0.75"
                                            max="1.45"
                                            step="0.05"
                                            value={brand.logoScale ?? 1}
                                            onChange={(e) => updateBrand(brand.id, 'logoScale', Number(e.target.value))}
                                            className="w-full accent-blue-600"
                                        />
                                        {brand.logoNormalizedAt && (
                                            <p className="mt-1 text-[10px] text-emerald-600">Auto-fitted. Save settings to publish this logo URL.</p>
                                        )}
                                    </div>
                                </div>
                            ))}
