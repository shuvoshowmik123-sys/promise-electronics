...
import { reportsApi, type ReportData, settingsApi } from "@/lib/api";
...
export default function AdminReportsPage() {
  ...
  const handleExportPDF = () => {
    ...
    const monthlyRows = reportData.monthlyFinancials.map(d =>
      '<tr><td style="font-size:9px;padding:4px;">' + d.name + '</td><td style="font-size:9px;padding:4px;">' + getCurrencySymbol() + d.income.toLocaleString() + '</td><td style="font-size:9px;padding:4px;">' + getCurrencySymbol() + d.expense.toLocaleString() + '</td><td style="font-size:9px;padding:4px;">' + d.repairs + '</td></tr>'
    ).join('');
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold" data-testid="text-total-revenue">{getCurrencySymbol()}{summary.totalRevenue.toLocaleString()}</div>
                  ...
                  {monthlyFinancials.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={monthlyFinancials}>
                        ...
                        <XAxis dataKey="name" />
                        <YAxis />
                        <Tooltip formatter={(value: number) => [`${getCurrencySymbol()}${value.toLocaleString()}`, '']} />
                        <Legend />
                        <Bar dataKey="income" fill="#0EA5E9" name={`Income (${getCurrencySymbol()})`} />
                        <Bar dataKey="expense" fill="#EF4444" name={`Expenses (${getCurrencySymbol()})`} />
                      </BarChart>
                    </ResponsiveContainer>
                  ) : (
                      ...
                      {activityLogs.map((log, i) => (
                        <div key={i} className="flex items-center justify-between border-b pb-2 last:border-0" data-testid={`log-item-${i}`}>
                          <div className="flex items-center gap-3">
                            <div className={`h-8 w-8 rounded-full flex items-center justify-center ${log.type === 'job' ? 'bg-blue-100' :
                                log.type === 'payment' ? 'bg-green-100' : 'bg-slate-100'
                              }`}>
                              <FileText className={`h-4 w-4 ${log.type === 'job' ? 'text-blue-500' :
