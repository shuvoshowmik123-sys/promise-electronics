...
| Warranty Claims | Patched Needs Retest | 2026-07-03 | `raw/warranty-claims-*.json` + screenshots | Phase 27C + 27C-Hotfix: native mobile branch — safe job refs (originalJobSafeRef / newJobSafeRef via LEFT JOIN), fixed linked filter (Boolean(newJobId)), in_repair/completed status colors, route order fixed, read routes now require warranty.view permission, check routes require warranty.view|warranty.create. Needs visual QA on 390×844, 430×932, 844×390 landscape, 1440×900 desktop. |
