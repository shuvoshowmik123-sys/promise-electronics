...
7. **R2d - `RELEASE-CONTROLLED-INTEGRATION-STAGING-GATE-CLOSE-01A`: CLOSED.** The eight repaired files were re-staged via individually named `git add` commands, with `docs/BOT.md` explicitly excluded from staging as required. The full 285-entry candidate was re-verified via `git ls-files`, split by command type: 272 of 272 additions present, 12 of 12 deletions correctly absent, `skills` correctly absent, zero extra paths, zero held Area Intelligence paths. All four required gates passed together for the first time in this lineage: `git diff --cached --check` (zero findings, down from 108 across 8 files), `tsc`, `vite build`, and `build:server` with zero `empty-import-meta` warnings. A working-directory drift during the reservation attempt was caught before any real work began, investigated, and corrected without touching the repository or its staged index. The complete candidate is now fully staged and fully gate-clean. Evidence: `mobile-qa/release-controlled-integration-staging-gate-close-01a/20260727-1711/REPORT.md`.
8. **R2e - Inspector-authorized commit: DONE.** Committed locally as `8bd25f3` on `main`, 283 files changed, 63,795 insertions, 8,034 deletions, matching the staged candidate exactly. Not pushed; `main` is one commit ahead of `origin/main`. Pre-commit review confirmed zero drift since gate-close, a clean whitespace gate, and a clean manual secret scan. Evidence: `mobile-qa/release-candidate-commit-01a/20260727-1724/REPORT.md`.
9. **R3 - `RELEASE-CLEAN-CLONE-CANDIDATE-PROOF-01A`: DONE — FAIL.** Fresh local clone of `8bd25f3`, `npm ci`, all four gates and full `npx vitest run` executed in isolation. Real totals: **`tsc` FAIL (2 errors)**, **vitest `26 failed | 327 passed | 3 skipped (356 total)`** — supersedes the inherited, never-verified 24/332/356 figure. Findings: (a) `tsc` fails because the 5 held Area Intelligence files' uncommitted `CustomerDistanceExplorer.tsx` lacks a `publicSettingsStatus` prop that committed `home.tsx` now passes — the exact modified-to-modified risk the ownership review flagged as unadjudicated; (b) 3 test failures trace to 2 proof `.mjs` scripts genuinely omitted from all 285 manifest commands (confirmed untracked, never staged); (c) 1 test failure is a real `db-baselines/.../manifest.json` hash mismatch against `schema.sql`/`promise-schema-migrations.sql`, both newly added in the same commit; (d) `auth-boundaries.test.ts` (3 skipped) is environmental — `validateEnv()` correctly rejecting a secrets-free clone, not a defect; (e) 22 failures are carried-forward pre-existing, unrelated to this candidate. Clone removed; primary worktree and Area Intelligence holdback proven untouched. Evidence: `mobile-qa/release-clean-clone-candidate-proof-01a/20260727-1734/REPORT.md`. **Not pushed, not migrated, not deployed.**
10. **R4 - `TEST-SUITE-RESTORATION-00A`: AUDIT COMPLETE.** Read-only failure-ownership audit closed; no repair performed. Every clean-clone finding now has a confirmed owner and repair contract: (1) `home.tsx`/held `CustomerDistanceExplorer.tsx` — smallest compile-safe fix is removing the two prop-pass lines in `home.tsx` (D1-neutral), or a new explicit D1 decision to land the richer behavior; (2) both omitted `.mjs` proof scripts are confirmed safe to add as-is (neither executes under vitest; both carry adequate safety guards) — a manifest completeness gap, not a deliberate exclusion; (3) the baseline `manifest.json` hash mismatch is a manifest-only data fix (two sha256 fields; actual values confirmed) with no regeneration tooling ever having existed; (4) all 22 remaining failures are individually cited and classified **candidate-induced test staleness** across 7 root-cause groups (new middleware/db-call exports missing from stale test mocks, a renamed geofence status, a retired legacy job-status name, and a self-referential `REQUIRED_MAIN_SCHEMA_VERSION` assertion anti-pattern repeated 3x) — all 22 fixes are test-only, zero require a source change; (5) the 3 `auth-boundaries` skips are an environment-contract gap (the one test file that boots the real app fail-closes without real secrets, correctly) — recommended fix is scoped local dummy env values, not real credentials. Evidence: `mobile-qa/test-suite-restoration-00a/20260727-1816/REPORT.md`. **Next:** a separately authorized corrective package (or packages) must implement the accepted contract — add the 2 scripts, fix the manifest hashes, apply the 22 test-only fixes, resolve the held-component decision, resolve the auth-boundaries gap — before the suite can be green and production release work resumes.
11. **R4a - `TEST-SUITE-RESTORATION-01A` (CANDIDATE-INTEGRITY-REPAIR): DONE — PASS 5 / FAIL 0 / NOT VERIFIED 0.** Repaired the 4 non-test candidate-integrity defects from the R4 audit, working-tree only (unstaged, no commit): (1) `home.tsx` — removed both `publicSettingsStatus` prop passes and the now-unused query destructures/derived value (D1-neutral Option A; zero held Area Intelligence files touched); (2) the 2 omitted `.mjs` proof scripts confirmed present and unmodified, left unstaged (no `git add`); (3) `db-baselines/.../manifest.json` — corrected exactly the 2 `sha256` fields to freshly-computed actual values, no other field or SQL file touched. **Isolated-clone proof** (fresh local clone of exactly `8bd25f3`, only the 4 scoped files copied in): `git diff --check` PASS, `tsc --noEmit` **PASS with zero errors** (both original errors gone), `vite build` PASS, `build:server` PASS, targeted vitest (`baseline-adoption-disposable.test.ts` + `reminders-prerequisite-reconciliation.test.ts`) **PASS, 25/25 tests**. Held D1 paths and primary `HEAD` proven untouched after cleanup. Evidence: `mobile-qa/test-suite-restoration-01a/20260727-1834/REPORT.md`. **Not claimed:** full suite green — the 22 test-staleness fixes and the `auth-boundaries` environment gap are explicitly deferred to `TEST-SUITE-RESTORATION-01B`. **Not committed, pushed, migrated, or deployed.**
12. **R4b - `TEST-SUITE-RESTORATION-01B` (TEST-CONTRACT-REPAIR): DONE — PASS 6 / FAIL 0 / NOT VERIFIED 0.** Repaired all 22 audited test-staleness failures (7 root-cause groups) and resolved the `auth-boundaries` environment gap (3 skips), test-only, working-tree only (unstaged, no commit), across the 11 allowed test files. Discovered and fixed several cascading gaps beyond the original audit once first-layer fixes let execution reach deeper real code: missing `requireCustomerAuth`/`getCustomerId`/`listServiceRequestsPaginated`/`getJobTicketsByIds`/`req.user` mocks; the *same* `db.transaction` root cause at a second call site (`transitionJobStatus`); a transitive `shared/schema.js` mock gap fixed by switching to an `importOriginal` merge; and 2 tests asserting a call to `jobService.recordJobPayment`, confirmed via `grep -rn` to be **dead code never called by the real server** — rewritten to assert the actual current call path (`settleJobPaymentViaPos`, canonical POS settlement). **Dirty-worktree:** `npx vitest run` → `356 passed | 0 failed | 0 skipped`. **Isolated-clone proof** (fresh clone of exactly `8bd25f3`, only the 4 `01A` candidate files + 11 `01B` test files copied in): all four build gates PASS, and **full `npx vitest run` (no filter): PASS — 356 passed / 0 failed / 0 skipped (356 total), exit 0** — the first fully green, fresh-clone-verified result for this candidate. Held D1 paths and primary `HEAD` proven untouched after cleanup. Evidence: `mobile-qa/test-suite-restoration-01b/20260727-1953/REPORT.md`. **Not committed, pushed, migrated, or deployed.** *(Evidence-accounting correction, 2026-07-27 20:38 Asia/Dhaka: the "22 audited failures" count above is correct and unchanged — the `transitionJobStatus` `db.transaction` fix and the 2 `recordJobPayment` rewrites are cascading defects found beneath already-counted Group-1 test cases, not additional distinct failures; the number 25 that appeared in the original evidence's per-item breakdown was a naive, incorrect sum, not a real count. See `mobile-qa/test-suite-restoration-01b/20260727-1953/EVIDENCE-CORRECTION-1.md`. No re-repair; the PASS result and all gate results are unchanged.)*
13. **R4c - `RELEASE-CORRECTIVE-COMMIT-01A`: FAIL — STOPPED, INDEX PRESERVED. No commit created.** Preflight passed (base verified at `HEAD=8bd25f3`/`main`/`ahead 1`; all 15 approved paths in expected state; all 5 held Area Intelligence paths excluded and untouched; the 2 `.mjs` scripts' identity re-verified by size+mtime only). Staged exactly the 15 approved paths via 15 individually named `git add` commands (no `.`/`-A`/globs) — `git diff --cached --name-only` confirmed exactly those 15. **Gate 1 (`git diff --cached --check`) FAILED:** `scripts/reminders-prerequisite-reconciliation-proof.mjs:331: new blank line at EOF` — a genuine, pre-existing extra trailing blank line in the file exactly as it has existed on disk since authorship (Jul 22), never touched by any package this session; first surfaced because this file had never previously been run through a staged whitespace gate. Per this repo's own precedent, this class of finding is not fixed inline — stopped before the secret scan and commit steps; **no `git reset`/`restore` was run**, the 15-path stage remains preserved. `HEAD` unchanged at `8bd25f3`; no push/amend/migration/deployment. Evidence: `mobile-qa/release-corrective-commit-01a/20260727-2045/REPORT.md`. **Next:** a separate, narrowly-scoped, explicitly authorized whitespace hotfix for that one file (remove the one extra trailing blank line, content-only), then re-attempt `RELEASE-CORRECTIVE-COMMIT-01A`.
14. **R4d - `RELEASE-REMINDERS-PROOF-WHITESPACE-HOTFIX-01A`: DONE — PASS.** Repaired the single pre-existing whitespace defect that blocked R4c: removed exactly one trailing LF byte from `scripts/reminders-prerequisite-reconciliation-proof.mjs` (14,721→14,720 bytes, 331→330 lines, `});\n\n`→`});\n`), working-tree only, confirmed by direct byte inspection. `git diff --check` on the file now PASSes. **The R4c 15-path staged index was left fully intact** — confirmed because `git diff --cached --check` still reports the original failure after this repair, proving the staged blob still holds the old content (no `git add` was run). `HEAD` unchanged at `8bd25f3`; no commit/push/deploy. Evidence: `mobile-qa/release-reminders-proof-whitespace-hotfix-01a/20260727-2053/REPORT.md`. **Next:** a corrective-commit recovery package that re-stages only this one repaired script, then re-runs the full staged gate, secret scan, and commit.
15. **R4e - `RELEASE-CORRECTIVE-COMMIT-RECOVERY-01A`: DONE — PASS, commit created.** Recovered the blocked R4c corrective commit: re-staged only `scripts/reminders-prerequisite-reconciliation-proof.mjs` (`git add -- <path>`, no other path touched), re-verified the staged set was still exactly the same 15 approved paths, ran `git diff --cached --check` (**PASS, 0 findings**), ran a manual structural secret scan over the full staged diff (**0 real secrets**; classified the 3 known loopback test dummy values in `tests/auth-boundaries.test.ts` as harmless test fixtures), then committed once: `test: restore release candidate integrity` → **`98a0775`** (`98a07757956597162a3a6f1e8aa46b2668ba8104`), **parent `8bd25f3`** exactly, **15 files changed, 642 insertions / 56 deletions**. Post-commit verification confirmed the file list matches the approved 15 exactly, all 5 held Area Intelligence paths and all docs/screenshots/`.grok`/`.env` content are absent from the commit, and `main` is now **`ahead 2`** of `origin/main`. Evidence: `mobile-qa/release-corrective-commit-recovery-01a/20260728-0103/REPORT.md`. **Not pushed, not migrated, not deployed.**
16. **R4f - `RELEASE-CLEAN-CLONE-CANDIDATE-PROOF-01A-R2`: DONE — PASS 8 / FAIL 0 / NOT VERIFIED 0.** Final fresh clean-clone proof against the actual committed candidate `98a0775` (not a working-tree approximation). Isolated local clone (no remote URL), explicit checkout, all 15 corrective paths confirmed tracked/committed before install, no `.env`/`node_modules`/screenshots/scripts copied from primary. Gates: `git diff 6c950a0f9d570b95b052719741297bfc67579229..HEAD --check` **PASS** (0 findings) · `tsc --noEmit` **PASS** (0 errors) · `vite build` **PASS** · `build:server` **PASS** · **full `npx vitest run` (no filter): PASS — 356 passed / 0 failed / 0 skipped (356 total), exit 0.** First time the full suite has been proven green against the real committed tree of the release candidate itself. Clone removed; primary `HEAD` and the 5 held D1 paths proven byte-identical before/after. Evidence: `mobile-qa/release-clean-clone-candidate-proof-01a-r2/20260728-0110/REPORT.md`. **Determination: candidate `98a0775` is release-ready for R5.** Not authorised by this package: push, migration, deployment, or production access.
17. **R5-Preflight - `PRODUCTION-RELEASE-AND-VERIFICATION-01A`: DONE — RELEASE PREFLIGHT PASS.** Confirmed candidate `98a0775` (parent `8bd25f3`, `main` ahead 2 of `origin/main`, unpushed, 0 staged changes) and re-verified the final clean-clone proof (all 5 gates PASS, 356/0/0). Identified without running: the trusted migration command (`MAIN_MIGRATION_RELEASE_MODE=true ALLOW_PROD_DB_MIGRATE_MAIN=true npm run db:migrate:main`) and the Render/Vercel commit-verification route plus health endpoint (`GET /api/health`). Confirmed exactly 2 commits would be pushed and that the 68 dirty/untracked working-tree entries are never transmitted by `git push` regardless of state. Produced a full release-control checklist (backup owner/timestamp requirement, exact commit to push, trusted migration command, Render/Vercel verification, full §17.3–§17.7 production smoke matrix). Evidence: `mobile-qa/production-release-and-verification-01a/20260728-0136/REPORT.md`. **Stopped at the required point: all 4 approvals (backup < 1hr, push, migration, deployment verification/smoke) are PENDING — none granted.** Production untouched, NOT DEPLOYED.
18. **R5 - Protected production release:** the only eligible next step once all 4 pending approvals are explicitly granted. Fresh production backup under one hour; trusted MAIN migration CLI; approved commit push; Render/Vercel commit verification; Section 17 role, core-flow, security, reload, and health smoke. Only this phase may declare deployment.
19. **`APPLICATION-DATABASE-TOPOLOGY-AND-READINESS-00A`: DONE — AUDIT COMPLETE (source/config only, 0 database access).** Confirmed the documented contract (`DATABASE_URL` = MAIN = Aiven; `BRAIN_DATABASE_URL` = Brain = Neon, correctly isolated to `server/brain/`) and found a genuine, unresolved contradiction: the active `.env` and an older `.env.production.local` snapshot both structurally classify `DATABASE_URL` as a **Neon-pattern host**, not Aiven — no Aiven-pattern host was found configured as an actual value anywhere in the repo. Only `.env.example`/`.env.render.example` are git-tracked (both empty templates); every real `.env*` is gitignored/local-only and is **not** proof of what Render's live dashboard actually runs. **This bears directly on the pending R5 backup/migration approvals** — confirm with the production operator which database engine "production" genuinely is before treating a backup or migration target as settled. Also confirmed: normal server startup never runs DDL in any environment (read-only ledger verification only; current head `2026_07_25_work_locations_table`, 48 migrations); defined a safe test matrix (remote read-only only after ownership confirmed, all create/write tests local-disposable-only); and confirmed zero desktop `.exe` packaging infrastructure exists (needs a separate phase). Evidence: `mobile-qa/application-database-topology-and-readiness-00a/20260728-0217/REPORT.md`.
20. **`DEVELOPMENT-NEON-MAIN-READONLY-HEALTH-01A`: DONE — verdict `SCHEMA_BEHIND` (read-only, 0 production access).** With explicit operator confirmation that Render production uses Aiven and the `.env` Neon target is dev/test-only, ran a read-only health/schema audit against the confirmed development Neon database. Connection confirmed real (PostgreSQL 17.10). MAIN ledger: 45/48 applied, missing exactly the 3 newest migration IDs (`commission_engine_tables`, `attendance_records_gps_columns`, `work_locations_table`), 0 unexpected entries. **Key finding: the physical schema those 3 migrations create already fully exists** (all 21 tables/4 indexes/7 columns checked are present) — this is a **ledger bookkeeping gap, not a missing-schema gap**. Since all 3 migrations are idempotent, re-running the trusted migration CLI against this dev database would be expected to safely backfill just the 3 missing ledger rows with no schema change — not attempted here (read-only only). Zero writes/DDL/migrations/fixtures; zero production Aiven/Render/Vercel/Brain access; zero secret values printed (verified via grep of the evidence directory). Evidence: `mobile-qa/development-neon-main-readonly-health-01a/20260728-1227/REPORT.md`. **Next (requires explicit authorization):** re-run the trusted migration CLI against this confirmed dev database to backfill the 3 ledger rows; successful creation/write testing remains local-disposable-only.
21. **`DEVELOPMENT-NEON-MAIN-LEDGER-RECONCILIATION-01A`: FAIL — migration failed, stopped immediately, no retry.** Attempted the authorized backfill: `NODE_ENV=development MAIN_MIGRATION_RELEASE_MODE=true npm run db:migrate:main` against the confirmed dev Neon database, run exactly once. **Failed on its first statement:** `cannot execute CREATE TABLE in a read-only transaction`. **Root cause confirmed:** this database's role/connection has `default_transaction_read_only=on` **ambiently** — every new session starts read-only regardless of intent, confirmed via a separate diagnostic `SHOW` before this package's own explicit `SET`. Not caused by this or any prior session package. Two possible explanations, neither resolvable here: `DATABASE_URL` may point at a Neon read-only replica/branch endpoint instead of the primary read-write one, or the role has an intentional read-only default applied — **resolving this needs Neon dashboard/role-configuration access, outside this agent's scope.** Ledger integrity confirmed unchanged after the failed attempt (still 45/48, same 3 IDs missing, 0 duplicates/extras, advisory lock cleanly released) — the attempt was safe and changed nothing. Zero production/Aiven/Render/Vercel/Brain access; zero retries; zero raw SQL/manual ledger writes; zero secret values printed. Evidence: `mobile-qa/development-neon-main-ledger-reconciliation-01a/20260728-1314/REPORT.md`. **Next:** Inspector/production-operator must resolve the read-only-connection question via Neon dashboard access before any further migration attempt.
22. **`DEVELOPMENT-NEON-MAIN-LEDGER-RECONCILIATION-01B`: DONE — PASS.** The Inspector supplied a corrected `DATABASE_URL` for the same confirmed dev Neon target (used only as an inline env var, never written to any file; evidence directory scanned and confirmed secret-free). Preflight confirmed `default_transaction_read_only=off` (genuinely write-capable, unlike `01A`'s credential) and the same 45/48 before-state. Ran `NODE_ENV=development MAIN_MIGRATION_RELEASE_MODE=true npm run db:migrate:main` exactly once — **SUCCESS**: all 3 target migrations applied cleanly. Read-only after-proof confirmed: **ledger 48/48**, head `2026_07_25_work_locations_table`, each of the 3 target IDs present exactly once (0 duplicates, 0 extras), and all previously-audited 21 tables/4 indexes/7 columns still present. **Verdict: READY.** No application UI or business-write testing claimed. Zero production Aiven/Render/Vercel/Brain access; zero business-data writes; zero source edits/commit/push/deploy. Evidence: `mobile-qa/development-neon-main-ledger-reconciliation-01b/20260728-1340/REPORT.md`. **Outstanding: the exposed dev Neon credential still needs rotation via Neon dashboard — not performed by this package (out of scope).** *(Evidence correction, 2026-07-28: the preflight was originally described as "fully read-only"/"zero raw SQL" — corrected to disclose one session-scoped, explicitly-rolled-back `CREATE TEMP TABLE` DDL capability probe, which created no persistent table/data/schema change. The trusted migration CLI remains the only persistent database change. Migration result, all counts, and the PASS verdict are unchanged. No queue scope change; credential rotation still not performed. See `mobile-qa/development-neon-main-ledger-reconciliation-01b/20260728-1340/EVIDENCE-CORRECTION-1.md`.)*
23. **`DEVELOPMENT-NEON-SANDBOX-CONNECTION-VALIDATION-01A`: DONE — verdict `CONNECTABLE_WRITE_CAPABLE` (read-only, 0 production access, 0 DDL of any kind).** A newly supplied, distinct `NEON_TEST_DATABASE_URL` (not `.env`'s `DATABASE_URL`, not the previously-reconciled dev Neon database) was validated: used only as an inline env var, never written to any file; evidence directory scanned and confirmed secret-free. Preflight passed (present, `NODE_ENV` not production, Neon-pattern host). Connected once, read-only checks only (`SELECT`/`SHOW`/`information_schema`/`pg_catalog` — explicitly no `CREATE TEMP TABLE` or any DDL this time), disconnected immediately. Result: PostgreSQL 18.4, `default_transaction_read_only=off`, database-level `CREATE` privilege confirmed true via read-only `has_database_privilege()` introspection (no actual `CREATE` run), 0 public tables (fresh/empty sandbox, expected). **Verdict: reachable and configured to support writes, but currently empty — no write performed.** Zero production Aiven/Render/Vercel/Brain access; zero access to the existing reconciled Neon database; zero business-row reads; zero source edits/commit/push/deploy. Evidence: `mobile-qa/development-neon-sandbox-connection-validation-01a/20260728-1422/REPORT.md`. **Not authorized by this package: creating application test records on this sandbox database — creation-flow testing remains local-disposable-only.**
24. **`LOCAL-DISPOSABLE-APPLICATION-SCHEMA-AND-CREATION-SMOKE-01A`: DONE — PASS (0 production access, 0 remote database access of any kind).** The actual application write test, not a connection-only check. Provisioned a fresh disposable local-only PostgreSQL 18 cluster (loopback-only, unused port, `trust` auth confined to the temporary cluster), restored the trusted local baseline, and ran the real trusted release migration CLI — **SUCCESS**, ledger 48/48, head `2026_07_25_work_locations_table`. Started the real application server with `BRAIN_DATABASE_URL` overridden to an unreachable placeholder host (DNS-failure log lines confirm zero real Brain contact, non-blocking by design). Confirmed readiness (`GET /api/ready` 200), authenticated via the existing documented local QA pattern (`server/seed.ts`'s automatic Super Admin seed — no bypass), obtained a CSRF token normally (`GET /api/admin/csrf-token`), and ran a full create → read → delete → verify-gone cycle on a tagged inventory item entirely through normal application HTTP APIs (201/200/204/404) — zero raw SQL, zero manual ledger edits. All 4 build/whitespace gates PASS. Full cleanup confirmed: app stopped, disposable database dropped, cluster stopped, data directory removed, both ports closed. Evidence: `mobile-qa/local-disposable-application-schema-and-creation-smoke-01a/20260728-1433/REPORT.md`. **This does not change the R5 production approvals, which remain all 4 PENDING.**
25. **`WINDOWS-SCHEMA-MIGRATION-LAUNCHER-01A`: DONE — PASS.** Packaged `tools/windows_schema_migration.py` into a tested Windows `.exe` (PyInstaller, `tools/packaging/`) for LOCAL and DEVELOPMENT-only migrations. Replaced "every remote target is production" with explicit `TargetMode` (local disposable / development remote / production remote-disabled) validated before any command launch; production remote always rejected in v1. Both allowed modes run `NODE_ENV=development`, never set `ALLOW_PROD_DB_MIGRATE_MAIN`; development remote requires explicit redacted-fingerprint confirmation. Still only launches the reviewed `npm run schema:audit:ledger`/`npm run db:migrate:main` — no independent Python SQL, no credential persistence. 27/27 Python unit tests pass. Found and fixed 2 real defects during verification: window too short (action buttons clipped off-screen) and frozen-exe repo-root resolution pointing into PyInstaller's temp bundle instead of the real checkout. Proved the actual built `.exe` through its normal UI path against a disposable local PostgreSQL cluster: ledger 31→48, URL field confirmed cleared, zero credential leakage in evidence (grep-verified). All 4 build gates + `git diff --check` PASS. Full cleanup confirmed. Evidence: `mobile-qa/windows-schema-migration-launcher-01a/20260728-1505/REPORT.md`. **Generated `.exe` not committed. Does not change R5 production approvals (still all 4 PENDING).**
26. **`WINDOWS-SCHEMA-MIGRATION-LAUNCHER-01A-HOTFIX-1`: DONE — PASS.** Fixed defect: Development remote mode accepted every non-local host, risking misclassifying an Aiven production URL as development. Restricted to hosts ending `.neon.tech` via `resolve_target_mode()`, rejected before any command is built (proven by tests asserting the subprocess factory is never called for Aiven-pattern/arbitrary/malformed hosts). Local disposable and production-block behavior unaffected. 31/31 Python tests pass. Rebuilt `.exe`; proved via real UI that local mode still preflights successfully and a fabricated dummy Aiven-style URL under Development remote is rejected before any connection, with Run Schema staying disabled. No real remote target used. All 4 build gates + `git diff --check` PASS. Full cleanup confirmed. Evidence: `mobile-qa/windows-schema-migration-launcher-01a-hotfix-1/20260728-1543/REPORT.md`. **Generated `.exe` not committed.**
27. **`SCHEMA-UPDATE-CONTROL-UX-01A`: DONE — PASS.** Completed `SchemaUpdateControl.tsx` so a Super Admin can request a reviewed schema update from Settings — records a durable request only, never DDL/shell/migration/child process. Added `schemaUpdateApi.requestUpdate` (POSTs only `{confirm, password}` to the existing reviewed endpoint). "Request update" shows only when no active run, ledger not blocked, pending migrations exist, and Super Admin. Password-confirmed dialog with explicit checkbox; password cleared on submit/success/error/cancel/unmount (verified live). Invalidates only `["schema-update-status"]`. No backup/export/DB-URL-input/provider-token/direct-migration-button added. Backend/runner/migration untouched. 5 new client-contract tests added to `tests/schema-update-control-plane.test.ts` (37/37 pass); 1 stale guard test in `tests/test_windows_schema_migration.py` updated to match the explicitly-requested feature (31/31 pass). Proved full flow on disposable local PostgreSQL at desktop 1440x900 + mobile 390x844/430x932: pending state, dialog, password cleared, request recorded, ledger unchanged (47→47, zero DDL), no overflow/overlap. All gates PASS. Full cleanup confirmed. Evidence: `mobile-qa/schema-update-control-ux-01a/20260728-1619/REPORT.md`.
28. **`PROMISE-SCHEMA-MIGRATION-TOOL-BACKUP-RESTORE-01A`: DONE — PASS.** Extended the existing `PromiseSchemaMigration.exe` (no new app) with Backup and Migrate (schema check -> verified pg_dump backup outside repo -> pg_restore --list + SHA-256 verify -> existing trusted migration -> ledger recheck; typed MIGRATE confirmation for non-local targets) and Restore Backup (native file picker -> SHA-256 + target-fingerprint verify -> typed RESTORE confirmation, always -> restore -> ledger recheck). No migration SQL in Python; credentials only in child-process env vars, never argv, zeroed after use; only on-disk write is a credential-free backup-metadata sidecar. 47/47 Python tests pass. Found and fixed 2 real pg_restore defects via actual end-to-end testing: missing required `--dbname` argument, and in-place `--clean` restore leaving a target database partially restored on a real FK/constraint ordering error — fixed with drop+recreate+plain-restore. Proved the full required sequence on the real rebuilt `.exe` against a disposable local database: baseline(31) -> backup+migrate(->48) -> restore(->31) -> migrate(->48). All build gates PASS. Full cleanup confirmed. Evidence: `mobile-qa/promise-schema-migration-tool-backup-restore-01a/20260728-1704/REPORT.md`.
29. **`PROMISE-SCHEMA-MIGRATION-TOOL-NEON-REMOTE-PROOF-01A`: DONE — schema check + Backup and Migrate PASS, Restore Backup honestly BLOCKED, final ledger 48/48 (required end state met).** Test-only (zero code changes) proof of the existing `.exe`'s real remote workflow against an operator-supplied disposable Neon TEST database via the real `.exe` UI (Development remote mode). Host confirmed `.neon.tech` before any write. Empty target initialized only with the approved schema-only baseline + baseline ledger (no business data). Schema check PASS; Backup and Migrate PASS with typed MIGRATE confirmation — real pg_dump backup verified (SHA-256 + pg_restore --list) outside the repo, then real migration: ledger 31->48. Restore Backup: SHA-256/fingerprint verification passed, typed RESTORE confirmed, but the real `dropdb` step failed because Neon reported the target still in use by another session — a genuine platform constraint, not a tool defect. Stopped immediately per instructions, no raw SQL or alternative method attempted; database left completely unaffected, still 48/48 (already the required end state). Every backup confirmed outside repo, credential-free, then deleted at cleanup (Neon database itself left untouched, not deleted). All build gates PASS; zero credential leakage in evidence (grep-verified). Evidence: `mobile-qa/promise-schema-migration-tool-neon-remote-proof-01a/20260728-1740/REPORT.md`.
30. **`PROMISE-SCHEMA-MIGRATION-TOOL-NEON-REMOTE-RESTORE-HOTFIX-01A`: DONE — Restore Backup PASS (48->31, twice independently verified), final "migrate again to 48" step honestly BLOCKED, Neon test database left at ledger 31 (not the requested 48/48).** Extended the existing `.exe` (`tools/windows_schema_migration.py` only) so Restore Backup can use PostgreSQL's own `dropdb --force` for Development remote, behind a new from-scratch two-factor confirmation (unchecked-by-default consent checkbox + typed RESTORE, showing only the target fingerprint, never the database name/host) — never handwritten SQL, never `pg_terminate_backend`. Also hardened `_sanitize_tool_output` to strip ALTER/GRANT/CREATE/DROP/INSERT/SELECT-FROM SQL patterns after a raw `pg_restore` statement leaked into a failure dialog during live testing, and changed `run_restore_and_recheck` to always report its read-only ledger recheck even on a reported failure. 54/54 Python tests pass. Live proof against the real Neon test database with the rebuilt `.exe`: `dropdb --force` succeeded where the prior phase's plain `dropdb` failed on the same "active connection" blocker; Restore Backup reported success and independently verified ledger 48->31. The required final proof step (migrate again to reach 48) is BLOCKED by a newly discovered, distinct finding: after the forced-drop-and-recreate cycle, the recreated database's session-level `search_path` resolves empty, breaking the project's own already-reviewed, out-of-scope unqualified `SELECT ... FROM promise_schema_migrations` queries in `main-schema-migrate.service.ts`/`ledger-reconciliation-audit.service.ts` — even though the restored data is present and correct (verified via schema-qualified SQL). No handwritten SQL workaround and no out-of-scope Node code changes were made; the tool's own Preflight gate correctly refused to proceed. All build gates PASS; zero credential leakage in evidence (grep + visual screenshot review). Neon test database left at ledger 31, not the requested 48/48 — disclosed as an honest deviation. Evidence: `mobile-qa/promise-schema-migration-tool-neon-remote-restore-hotfix-01a/20260728-1823/REPORT.md`.
31. **`PROMISE-SCHEMA-MIGRATION-TOOL-NEON-SEARCH-PATH-ROOT-CAUSE-00A`: DONE — audit-only, verdict ROOT CAUSE UNRESOLVED.** Investigated the contradiction between the 17:40 report (claimed empty `search_path` yet successful audit+migration) and the 18:23 report (empty `search_path` blocked preflight after restore). Zero source edits, zero EXE rebuild, zero database writes. Source trace confirmed the exact mechanism: two unqualified `SELECT ... FROM promise_schema_migrations` queries (`server/services/main-schema-migrate.service.ts:2052`, `server/services/ledger-reconciliation-audit.service.ts:278`) fail under an empty `search_path`, mapping deterministically to the exact blocked message seen — the Python tool itself has zero `search_path`/`PGOPTIONS` references (grep-confirmed), and `dotenv` does not override an already-set `DATABASE_URL` (verified directly). Real EXE Preflight-only proof (no rebuild, Backup/Restore never clicked) now **PASSES** on the identical database, same target fingerprint, directly contradicting the 18:23 block with zero changes in between. Read-only reconciliation found identical zero-override state (`pg_db_role_setting` 0 rows, empty `rolconfig`) in both the 18:23 check and now, yet the resolved `search_path` value differs (empty then, normal now) — no persistent configuration difference exists to explain this. Verdict: not CONFIRMED (no persistent difference found), not REFUTED (`search_path` fully explains the 18:23 failure mechanism), **UNRESOLVED** (the transient trigger, most plausibly Neon connection-pooler behavior right after a `dropdb`/`createdb` cycle, cannot be reproduced without a mutating phase, out of scope here). Also noted: the 17:40 report's own claim of simultaneous empty `search_path` + successful unqualified reads is internally inconsistent under real Postgres semantics. Only `git diff --check` PASS (the sole authorized gate). No code fix proposed. Evidence: `mobile-qa/promise-schema-migration-tool-neon-search-path-root-cause-00a/20260728-1920/REPORT.md`.
32. **`PROMISE-SCHEMA-MIGRATION-TOOL-NEON-SEARCH-PATH-HARDENING-01A`: DONE — PASS.** Final proof for the existing `PromiseSchemaMigration.exe` (no new tool). Fixed the fragile `search_path` dependence confirmed by item 31: added a session-only `SET search_path TO public` in `runMainSchemaMigrations()` (`server/services/main-schema-migrate.service.ts`) immediately after connect, before the advisory lock / ledger table work / every migration body — never `ALTER DATABASE`/`ALTER ROLE`, never persisted; schema-qualified the two live-ledger reads in `verifyMainSchemaLedger()` (same file) and `readLiveLedgerChecksumMap()` (`server/services/ledger-reconciliation-audit.service.ts`) to `public.promise_schema_migrations`. No migrations/baseline SQL/frontend/routes/schema tables/product UI/Aiven-Render config/production-mode rules touched; no repository-wide cleanup; `tools/windows_schema_migration.py` inspected and confirmed unnecessary to change (no EXE rebuild). 4 new focused tests added to `tests/ledger-audit-startup-ownership.test.ts` (19/19 pass); zero regressions in the 2 other test files importing these services (58/58 pass); 77/77 total. All 5 gates PASS (focused tests, tsc, vite build, build:server, git diff --check). Real EXE proof (no rebuild) against the dedicated Neon TEST database: starting-state gate confirmed ledger exactly 31, then the full **31->48->31->48** sequence completed end-to-end — Test/Preflight PASS, Backup and Migrate (typed MIGRATE) 31->48 healthy, Restore Backup (SHA/fingerprint verified, two-factor consent checkbox confirmed unchecked-by-default then checked, typed RESTORE) 48->31 with "Ledger recheck: pending_only" succeeding immediately for the first time in this proof lineage (previously "unavailable"), and Backup and Migrate again 31->48 healthy — the exact step BLOCKED in the prior hotfix phase now completes cleanly. Final ledger independently confirmed at 48 via schema-qualified read-only query, as required. Forced remote restore stayed Development remote/`.neon.tech` only; two-factor confirmation not weakened; no handwritten SQL workaround; only this phase's 2 backup pairs deleted; zero credential leakage (grep + visual review). Evidence: `mobile-qa/promise-schema-migration-tool-neon-search-path-hardening-01a/20260728-2005/REPORT.md`.
33. **`PROMISE-SCHEMA-MIGRATION-TOOL-AIVEN-TEST-COMPATIBILITY-01A`: DONE — PARTIAL PASS WITH HONEST BLOCKER.** Added a new "Aiven test (session approved)" mode to the existing `.exe` (`tools/windows_schema_migration.py` only, no new tool), gated behind an in-memory SHA-256 fingerprint comparison against the session-approved `AIVEN_TEST_DATABASE_URL` (host pattern alone never sufficient; stops before any connection if unproven). Visually distinct (blue/bold style) from Neon development and disabled Production remote. Restore reuses the same reviewed forced-drop + two-factor confirmation pattern, parameterized by mode. 13 new focused tests (67/67 total pass, 54 pre-existing unmodified); TS-guard-test gate N/A by design (no TS source touched); all other gates PASS. Live proof against the real dedicated Aiven TEST database: existing Development remote mode correctly rejects the Aiven URL before any connection (unweakened safety boundary); new Aiven Test mode accepts only the approved target; Preflight PASS; Backup and Migrate 31->48 PASS. Restore Backup is honestly BLOCKED: `dropdb`'s required maintenance-database connection is rejected by Aiven's own `pg_hba.conf` for this service tier, confirmed deterministic via direct read-only diagnostic -- not a tool defect, no retry/manual-repair/workaround attempted. Migrate-again not needed (DB never left 48). Final ledger independently confirmed at 48. Along the way: fixed a Node-vs-libpq `sslmode` connection-string detail (translated `no-verify`->`require` only for libpq-facing env, not the safety design), and found+immediately fixed a critical live sanitizer gap (`dropdb` connection-failure text leaking hostname/IP/username/database name) -- verified zero-leak after rebuild. Zero credential leakage in final evidence (one screenshot showing the database name was removed before finalizing; grep + visual review clean). Evidence: `mobile-qa/promise-schema-migration-tool-aiven-test-compatibility-01a/20260728-2040/REPORT.md`.
34. **`PROMISE-SCHEMA-MIGRATION-TOOL-AIVEN-RESTORE-BOUNDARY-HOTFIX-01A`: DONE — PASS.** Corrected Aiven Test (session approved) mode so it offers only Test/Preflight and Backup and Migrate (the capabilities item 33 proved safe) -- Restore Backup is now disabled before any file picker, backup verification, pg_restore, dropdb, createdb, or database connection can begin, showing "Aiven restore is provider-controlled. Use Aiven Console Fork & Restore." `_restore_backup()` returns immediately for `TargetMode.AIVEN_TEST_APPROVED` as its literal first line; a new `_update_restore_availability()` disables the button and shows a persistent notice, wired into the mode-change trace and into `_set_busy()` (fixed so busy->idle doesn't re-enable it). Local disposable and Neon Development restore, the Aiven exact-fingerprint gate, Production remote's disabled state, and Aiven Backup and Migrate are all unchanged. Corrected a misleading SSL comment (libpq `require` vs node-pg `no-verify` are not unconditionally identical -- libpq upgrades to CA validation when a root cert is supplied) with zero logic change. 10 new focused tests (77/77 total pass, 67 pre-existing unmodified), including 6 real-`tk.Tk()` behavioral tests. All 5 gates PASS. Real EXE proof (rebuilt): Restore Backup visibly disabled with the exact message; clicking it produced zero new windows and zero dropdb/createdb/pg_restore/pg_dump processes; Preflight succeeded; optional Backup and Migrate (DB already healthy at 48) applied zero pending migrations and the restore restriction survived the full busy/idle cycle; ledger independently confirmed unchanged at 48. One backup created this phase deleted at cleanup. A temporary screenshot that briefly leaked the database name/fingerprint/SHA-256 prefix was caught and deleted before use, replaced with a cropped redacted version; final evidence scan clean. Evidence: `mobile-qa/promise-schema-migration-tool-aiven-restore-boundary-hotfix-01a/20260728-2130/REPORT.md`.
35. **`PROMISE-SCHEMA-MIGRATION-TOOL-AIVEN-RESTORE-CORE-GUARD-01A`: DONE — PASS.** Final narrow safety-close package: made Aiven Test restore impossible in both the EXE UI (unchanged from item 34) and the underlying Python restore functions. `run_restore()` and `run_restore_and_recheck()` (`tools/windows_schema_migration.py`) both return a safe blocked outcome for `TargetMode.AIVEN_TEST_APPROVED` as their literal first statement -- before `_find_pg_tool`, backup verification, file operation, subprocess, or database connection; `run_restore_and_recheck` also never runs the ledger recheck for this mode. Visible behavior unchanged (disabled Restore Backup button + exact "Aiven restore is provider-controlled. Use Aiven Console Fork & Restore." message). Local disposable and Neon Development restore unaffected. Replaced/updated 2 obsolete tests that expected Aiven restore to call `dropdb --force`; added 2 new tests proving the core guard directly (no `_find_pg_tool` call, no subprocess factory call, no `verify_backup_for_restore`/recheck call, exact message returned). 78/78 tests pass; all 5 gates PASS. Rebuilt the EXE; local-only UI check (no remote URL, no backups, no migrations) confirmed: Aiven Test mode shows disabled restore + exact message, clicking it opens zero new windows, switching back to Local disposable re-enables restore. No remote connection ever opened this phase. Evidence: `mobile-qa/promise-schema-migration-tool-aiven-restore-core-guard-01a/20260729-0015/REPORT.md`. **Only a separate, explicit commit/push decision remains.**
...
### RELEASE-COMMIT-SEGMENTATION-01A - COMPLETE

**2026-08-01 12:25 Asia/Dhaka.** PASS 10 / FAIL 0 / NOT VERIFIED 0. The exact 68-path inventory was converted into ten coherent content commits on `main`: 66 paths committed, with `client/src/components/customer/CustomerDistanceExplorer.tsx` and its D1-held `client/src/lib/area-notice.ts` companion intentionally excluded. This completion record is the separate 11th documentation-only commit. TypeScript passed after every content commit. Final gates passed: Vite, server build, whitespace checks, and full Vitest (**29 files, 379 passed, 0 failed, 0 skipped**). No D1-held path was committed; `main` is 11 commits ahead of `origin/main`; nothing was pushed, deployed, migrated, or run against production/database. Evidence: `mobile-qa/release-commit-segmentation-01a/20260801-1225/REPORT.md` and `results.json`. Next: owner decision on the separate secret-purge force-push and deployment sequence.
