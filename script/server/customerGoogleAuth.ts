...
async function upsertCustomerFromGoogle(profile: Profile) {
  ...
  const customer = await storage.upsertCustomerFromGoogle({
  ...
  passport.use(
    new GoogleStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
        try {
          const customer = await upsertCustomerFromGoogle(profile);
          done(null, { customerId: customer.id, authMethod: 'google' as const });
        } catch (error) {
  ...
  app.get("/api/customer/auth/me", async (req: any, res) => {
    try {
      ...
      if (req.session?.customerId && !req.session?.authMethod) {
        const customer = await storage.getCustomer(req.session.customerId);
        if (customer) {
          // Only trust if customer doesn't have a googleSub (i.e., they are a phone user)
