...
export async function registerRoutes(
  ...
  // Due Records API
  app.get("/api/due-records", async (req, res) => {
    try {
  ...
  app.post("/api/due-records", async (req, res) => {
    try {
      const validated = insertDueRecordSchema.parse(req.body);
      const record = await storage.createDueRecord(validated);
      res.status(201).json(record);
    } catch (error) {
  ...
  app.patch("/api/due-records/:id", async (req, res) => {
    try {
      const record = await storage.updateDueRecord(req.params.id, req.body);
      if (!record) {
  ...
  app.delete("/api/due-records/:id", async (req, res) => {
    try {
      const success = await storage.deleteDueRecord(req.params.id);
      if (!success) {
        return res.status(404).json({ error: "Due record not found" });
      }
      res.status(204).send();
    } catch (error) {
      res.status(500).json({ error: "Failed to delete due record" });
    }
  });
  ...
  app.post("/api/customer/register", async (req, res) => {
    try {
      ...
      const existingCustomer = await storage.getCustomerByPhone(validated.phone);
      if (existingCustomer) {
      ...
      await storage.linkServiceRequestsByPhone(validated.phone, customer.id);
  ...
  app.post("/api/customer/login", async (req, res) => {
    try {
      ...
      const customer = await storage.getCustomerByPhone(validated.phone);
      if (!customer) {
  ...
  app.post("/api/customer/service-requests/link", requireCustomerAuth, async (req, res) => {
    try {
      ...
      const customer = await storage.getCustomer(req.session.customerId!);
      if (!customer || customer.phone !== order.phone) {
        return res.status(403).json({ error: "Phone number does not match order" });
      }

      const linked = await storage.linkServiceRequestToCustomer(order.id, customer.id);
      res.json(linked);
    } catch (error) {
  ...
  app.get("/api/customer/warranties", requireCustomerAuth, async (req, res) => {
    try {
      ...
      const jobs = await storage.getJobTicketsByCustomerPhone(customer.phone);
  ...
  app.patch("/api/admin/customers/:id", requireAdminAuth, async (req, res) => {
    try {
      ...
      const updated = await storage.updateCustomer(req.params.id, updates);
      if (!updated) {
        return res.status(404).json({ error: "Customer not found" });
      }
