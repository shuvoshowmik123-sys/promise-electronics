...
export interface IStorage {
  ...
  getJobTicket(id: string): Promise<JobTicket | undefined>;
  getNextJobNumber(): Promise<string>;
  createJobTicket(job: InsertJobTicket): Promise<JobTicket>;
  updateJobTicket(id: string, updates: Partial<InsertJobTicket>): Promise<JobTicket | undefined>;
  deleteJobTicket(id: string): Promise<boolean>;
  getJobTicketsByCustomerPhone(phone: string): Promise<JobTicket[]>;
  ...
  getWebsiteInventoryItems(): Promise<InventoryItem[]>;
  ...
  updateChallan(id: string, updates: Partial<InsertChallan>): Promise<Challan | undefined>;
  deleteChalan(id: string): Promise<boolean>;

  // Petty Cash
  getAllPettyCashRecords(): Promise<PettyCashRecord[]>;
  createPettyCashRecord(record: InsertPettyCashRecord): Promise<PettyCashRecord>;
  deletePettyCashRecord(id: string): Promise<boolean>;

  // Due Records
  getAllDueRecords(): Promise<DueRecord[]>;
  createDueRecord(record: InsertDueRecord): Promise<DueRecord>;
  updateDueRecord(id: string, updates: Partial<InsertDueRecord>): Promise<DueRecord | undefined>;
  deleteDueRecord(id: string): Promise<boolean>;
  ...
  getServiceRequestByTicketNumber(ticketNumber: string): Promise<ServiceRequest | undefined>;
  createServiceRequest(request: InsertServiceRequest & { customerId?: string | null; expiresAt?: Date | null }): Promise<ServiceRequest>;
  updateServiceRequest(id: string, updates: Partial<InsertServiceRequest> & { trackingStatus?: string; expiresAt?: Date | null; stage?: string }): Promise<ServiceRequest | undefined>;
  deleteServiceRequest(id: string): Promise<boolean>;
  getExpiredServiceRequests(): Promise<ServiceRequest[]>;
  ...
  linkServiceRequestsByPhone(phone: string, customerId: string): Promise<number>;

  // Customers (Customer Portal Auth)
  getAllCustomers(): Promise<Customer[]>;
  getCustomer(id: string): Promise<Customer | undefined>;
  getCustomerByPhone(phone: string): Promise<Customer | undefined>;
  getCustomerByEmail(email: string): Promise<Customer | undefined>;
  getCustomerByGoogleSub(googleSub: string): Promise<Customer | undefined>;
  createCustomer(customer: InsertCustomer): Promise<Customer>;
  updateCustomer(id: string, updates: Partial<InsertCustomer>): Promise<Customer | undefined>;
  upsertCustomerFromGoogle(data: UpsertCustomerFromGoogle): Promise<Customer>;
...
export class DatabaseStorage implements IStorage {
  ...
  async createPosTransaction(transaction: InsertPosTransaction): Promise<PosTransaction> {
    ...
    try {
      if (paymentStatus === "Due") {
        ...
        await db.insert(schema.pettyCashRecords).values({
          id: nanoid(),
          description: `POS Sale - ${invoiceNumber} (${paymentMethod})`,
          category: `POS-${paymentMethod}`,
          amount: transaction.total,
          type: "Income",
        });
      }
    } catch (financeError) {
      console.error("Failed to create finance record for transaction:", invoiceNumber, financeError);
    }
  ...
  async getCustomerByGoogleSub(googleSub: string): Promise<Customer | undefined> {
    const [customer] = await db.select().from(schema.customers).where(eq(schema.customers.googleSub, googleSub));
    return customer;
  }

  async createCustomer(customer: InsertCustomer): Promise<Customer> {
    const [newCustomer] = await db.insert(schema.customers).values({ ...customer, id: nanoid() }).returning();
    return newCustomer;
  }
  ...
  async upsertCustomerFromGoogle(data: UpsertCustomerFromGoogle): Promise<Customer> {
    // First check if customer exists with this Google ID
    const existing = await this.getCustomerByGoogleSub(data.googleSub);
    if (existing) {
      // Update existing customer with latest info from Google
      const [updated] = await db
        .update(schema.customers)
        .set({
          name: data.name,
    ...
    // Check if customer exists with same email (link existing account)
    if (data.email) {
